...
namespace VisualDebugger
{
	...
	PhysicsEngine::Trampoline* trampoline;
	PhysicsEngine::Sphere* sphere;
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		//Adds trampoline to the initialised scene
		trampoline = new PhysicsEngine::Trampoline();
	...
	{
		PxTransform Camera_pos = camera->getTransform(); //gets camera position
		PxVec3 Camera_dir = camera->getDir(); //gets camera direction
		switch (toupper(key))
		{
		//implement your own
		case 'R':
			...
			break;
		case 'T':
			sphere = new PhysicsEngine::Sphere(Camera_pos);
			sphere->Color(PxVec3(0.f, 0.f, 0.f));
			scene->Add(sphere);
			((PxRigidBody*)sphere->Get())->addForce(PxVec3((Camera_dir.x * 500), (Camera_dir.y * 500), (Camera_dir.z * 500)) * gForceStrength);
			break;
		default:
			break;
		}
