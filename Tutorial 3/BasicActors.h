...
namespace PhysicsEngine
{
	...
	class DivingBoard : public DynamicActor
	{
	public:
		DivingBoard(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(2.f, 2.f, 2.f), PxReal density = 7860.f) //7860 kg/m^3 is irons density
			: DynamicActor(pose)
		{
			CreateShape(PxBoxGeometry(PxVec3(dimensions.x, (dimensions.y * 2.5f), dimensions.z)), density); //is parameterised to change acoring to dimensions 
			CreateShape(PxBoxGeometry(PxVec3(dimensions.x, (dimensions.y / 10.f), dimensions.z)), density);
			GetShape(0)->setLocalPose(PxTransform(PxVec3(.0f, .0f, .0f)));
			GetShape(1)->setLocalPose(PxTransform(PxVec3((dimensions.x * 2.f), (dimensions.y * 2.4f), .0f)));
		}
	};
	...
	{
	public:
		CompoundObject(const PxTransform& pose = PxTransform(PxIdentity), float angle = 0, PxVec2 dimensions = PxVec2(1.f, 1.f), PxReal density = 1.f)
			: DynamicActor(pose)
		{
			for (float i = 0; i < 3; i++)
			{
				CreateShape(PxCapsuleGeometry(dimensions.x, dimensions.y), density);
				//float deg = angle * PxPi / 180;
				//GetShape(0)->setLocalPose(PxTransform(PxVec3(0.f, .0f, .0f), PxQuat((PxPi / 4.f), PxVec3(0.f, 0.f, 1.f))));
			}
			CreateShape(PxCapsuleGeometry(dimensions.x, dimensions.y), density);
			GetShape(0)->setLocalPose(PxTransform(PxVec3(3.f, 0.f, 0.f)));
			GetShape(1)->setLocalPose(PxTransform(PxVec3(0.f, 3.f, 0.f), PxQuat((PxPi / 2.f), PxVec3(0.f, 0.f, 1.f))));
			GetShape(2)->setLocalPose(PxTransform(PxVec3(-3.f, 0.f, 0.f), PxQuat(PxPi, PxVec3(0.f, 0.f, 1.f))));
			GetShape(3)->setLocalPose(PxTransform(PxVec3(0.f, -3.f, 0.f), PxQuat(-(PxPi / 2.f), PxVec3(0.f, 0.f, 1.f))));
		}
	};
	...
	public:
		...
		{
			PxRigidActor* px_actor0 = 0;
			//PxRigidDynamic* px_actor0 = 0;

			if (actor0)
				//px_actor0 = (PxRigidDynamic*)actor0->Get();
				px_actor0 = (PxRigidActor*)actor0->Get();
