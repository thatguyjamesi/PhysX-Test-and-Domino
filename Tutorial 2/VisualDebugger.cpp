...
namespace VisualDebugger
{
	...
	void UserKeyPress(int key)
	{
		...
		PxVec3 Camera_dir = camera->getDir();
		switch (toupper(key))
		{
		//implement your own
		case 'R':
			//Spawns an object at the cameras postition
			sphere = new PhysicsEngine::Sphere(Camera_pos);
			sphere->Color(PxVec3(0.f, 0.f, 0.f));
			scene->Add(sphere);
			((PxRigidBody*)sphere->Get())->addForce(PxVec3((Camera_dir.x * 500), (Camera_dir.y *500), -500) * gForceStrength);
			break;
		default:
			break;
		}
