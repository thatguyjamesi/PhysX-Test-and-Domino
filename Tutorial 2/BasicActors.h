...
namespace PhysicsEngine
{
	...
	public:
		...
		// - denisty: 1kg/m^3
		//Box(const PxTransform& pose = PxTransform(PxIdentity), const PxTransform& angle = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = 1.f)
	...
	{
	public:
		Enclosure(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = 1.f)
			: DynamicActor(pose) 
		{
			CreateShape(PxBoxGeometry(dimensions), density);
			CreateShape(PxBoxGeometry(dimensions), density);
			GetShape(0)->setLocalPose(PxTransform(PxVec3(-.5f, .0f, .0f)));
			GetShape(1)->setLocalPose(PxTransform(PxVec3(.5f, .0f, .0f)));
		}	
	};
